...
def placePic1(img_show, text1="原始图像"):
    ...
    myWindow.label3.place(
        relx=0.25, rely=0.40, width=picSize, height=picSize, anchor=tkinter.CENTER
    )  # 设置绝对座标
...
def placePic2(img_show, text2="处理结果图", RGB=True):
    ...
    myWindow.label4.place(
        relx=0.75, rely=0.40, width=picSize, height=picSize, anchor=tkinter.CENTER
    )
...
def Convert_HSV():
    ...
    if img_open is not None:
        ...
        myWindow.explainText.set(
            "HSV彩色模型：H(Hue)色度 、S(Saturation)饱合度、V(Value)亮度\n"
...
def Savepic():
    ...
    global img_result
    if img_result is not None:
        fname = asksaveasfilename(title="保存文件", filetypes=[("JPG", ".jpg")])
        if fname:
...
def Negative():
    ...
    if img_gray is not None:
        ...
        myWindow.explainText.set(
            " 图像反转用于增强图像暗色区域的白色或灰色细节，适用于黑色较多的图像增强。\n "
...
def Piecewise_linear_transformation():
    ...
    if img_gray is not None:
        ...
        if n_int:
            ...
            img_result = PointProcessing.piecewise_linear_transformation(
                img_gray.copy(), paraW.paraLists
            )
...
def Logarithmic_transformations():
    ...
    if img_gray is not None:
        ...
        placePic1(img_gray, "原灰度图像")
        paraC = askfloat(
            "参数设置", "s=C*log(1+r),请输入对数变换常数C", initialvalue=20.0
        )
...
def Power_law_transformations():
    ...
    if img_gray is not None:
        ...
        img_result = PointProcessing.power_law_transformations(
            img_gray.copy(), paraGamma
        )
...
def Gray_histogram():
    ...
    if img_gray is not None:
        ...
        binSize = askinteger(
            title="直方图参数设置", prompt="请输入直方图的灰度级数", initialvalue=256
        )
        placePic1(img_gray, "原灰度图像")
...
def Mask_histogram():
    ...
    if img_gray is not None:
        ...
        if pathMask != 0 and pathMask != "":
            ...
            histr, masked_img = Histogram.mask_histogram(
                img_gray.copy(), imgMask, binSize
            )
...
def Equalization_histogram():
    ...
    if img_gray is not None:
        ...
        myWindow.explainText.set(
            """直方图均衡化是一种常用的灰度增强算法，也称为直方图均匀化。它是将原图像的直方图经过变换函数修整为均匀直方图，使得图像的灰度分布趋向均匀。 """
        )
    else:
        showwarning(title="警告", message="请先在文件菜单下打开图片！")
...
def SML():
    ...
    dest = ["正三角形", "倒三角形", "均匀分布"]
    typeHistgram = askinteger(
        "选择目标直方图",
        "请选择目标直方图，1=正三角形，2-倒三角形，3=均匀分布",
        initialvalue=1,
        minvalue=1,
        maxvalue=3,
    )
...
def GML():
    ...
    if img_gray is not None:
        ...
        if typeHistgram == 1 or typeHistgram == 2 or typeHistgram == 3:
            histr, equ = Histogram.regulation_histogram(
                img_gray.copy(), typeHistgram, False
            )
            myWindow.showAndBarFig0(equ, histr)
...
def Gaussian_filter():
    ...
    kernalSize = askinteger(
        "设置滤波核大小", "请输入高斯滤波核的大小（通常为奇数）", initialvalue=3
    )
...
def Median_filter():
    ...
    if img_result is not None:
        answer = askyesno(
            "选择待滤波图像", "Y-对打开图像进行滤波，N-对处理后图像进行滤波"
        )
        if not answer:
            ...
            img_ = img_gray.copy()
    myWindow.explainText.set(
        "中值滤波的方法是：对邻域内的像素灰度值按大小排序，然后取中间位置的灰度值取代当前像素的灰度值。"
        "\n中值滤波对滤除脉冲干扰及图像扫描噪声最为有效，它可以滤除小于1/2窗口的脉冲信号。"
    )
    placePic1(img_, "原灰度图像")
    kernalSize = askinteger(
        "设置滤波核", "请输入中值滤波核的大小（通常为奇数）", initialvalue=3
    )
    if kernalSize and kernalSize % 2 == 1:
...
def Max_filter():
    ...
    if img_result is not None:
        ...
        if not answer:
            ...
            img_ = img_gray.copy()
    myWindow.explainText.set(
        "最大值滤波的方法是：对邻域内的像素灰度值按大小排序，然后取最大的灰度值取代当前像素的灰度值。"
        "\n最大值滤波适合去除胡椒噪声。"
    )
    placePic1(img_, "原灰度图像")
...
def Min_filter():
    ...
    if img_result is not None:
        ...
        if not answer:
            ...
            img_ = img_gray.copy()
    myWindow.explainText.set(
        "最小值滤波的方法是：对邻域内的像素灰度值按大小排序，然后取最小的灰度值取代当前像素的灰度值。"
        "\n最小值滤波适合去除盐噪声。"
    )
    placePic1(img_, "原灰度图像")
...
def Smooth_filter():
    ...
    if img_result is not None:
        ...
        if not answer:
            ...
            img_ = img_gray.copy()
    myWindow.explainText.set(
        "自定义滤波可以由用户定义滤波核的大小和权重，\n并根据定义的滤波器进行图像的滤波处理。"
    )
    placePic1(img_, "原灰度图像")
    kernalSize = askinteger(
        "设置滤波核", "请输入滤波核的大小（通常为奇数）", initialvalue=3
    )
    if kernalSize and kernalSize % 2 == 1:
        temp = [kernalSize * [1]]
        ...
        paraW = paraWindow2(
            ...
            paraLists,
            "滤波核设置",
            "请设置" + str(kernalSize) + "x" + str(kernalSize) + "滤波核的权重",
        )
...
def Constant_border():
    ...
    myWindow.explainText.set(
        """BORDER_CONSTANT：常量，增加的变量值为value色 [value][value] | abcdef | [value][value]
...
"""
    ...
    if borderSize:
        img_result = SpatialFilter.border_fill(
            img_gray.copy(), cv2.BORDER_CONSTANT, borderSize
        )
        placePic2(img_result)
...
"""
    ...
    if borderSize:
        img_result = SpatialFilter.border_fill(
            img_gray.copy(), cv2.BORDER_REPLICATE, borderSize
        )
        placePic2(img_result)
...
"""
    ...
    if borderSize:
        img_result = SpatialFilter.border_fill(
            img_gray.copy(), cv2.BORDER_REFLECT, borderSize
        )
        placePic2(img_result)
...
"""
    ...
    if borderSize:
        img_result = SpatialFilter.border_fill(
            img_gray.copy(), cv2.BORDER_REFLECT101, borderSize
        )
        placePic2(img_result)
...
"""
    ...
    if borderSize:
        img_result = SpatialFilter.border_fill(
            img_gray.copy(), cv2.BORDER_WRAP, borderSize
        )
        placePic2(img_result)
...
def Laplacian_filter():
    ...
    picLists = [
        ["Laplacian是二阶微分算子，以下为四邻域模板", imgExplain1],
        ["Laplacian的八邻域模板具有方向同性", imgExplain2],
    ]
...
def Magnitude_spectrum0():
    ...
    text = (
        "为了提高幅值谱的可视化程度，对幅值谱按公式y=20*log(1+|F(u,v)|)进行了非线性变换"
    )
...
def Phase_spectrum0():
    ...
    text = (
        "傅里叶变换的幅值谱是对信号轮廓和形状的描述，而相位谱是对信号位置的描述。\n"
...
def Inverse_fourier():
    ...
    img_result = Fourier.ifft(
        dft
    )  # 　由于傅里叶变换期间，使用了最优尺寸，反变换时需要恢复原图像大小
    placePic2(img_result, "傅里叶反变换")
    myWindow.explainText.set(
        "此操作是先对图像进行傅里叶变换，再进行逆变换。\n"
...
def Idle_low_pass():
    ...
    picLists = [
        [
            "理想低通滤波器是将小于截止频率的分量完全\n无损地通过，而大于截止频率的分量完全衰减。",
            imgExplain0,
        ],
        ["理想低通滤波器示意图", imgExplain1],
    ]
    myWindow.showFig1(picLists)
    placePic1(img_gray, "原灰度图像")
    ...
    d0 = askinteger(
        "理想低通滤波器参数设置",
        "请设置截止频率，其值在[" + str(1) + "~" + str(maxD0) + "]之间",
        initialvalue=(1 + maxD0) // 2,
    )
...
def Butterworth_low_pass():
    ...
    picLists = [
        [
            "巴特沃斯低通滤波器的通带与阻带之间没有明显的\n不连续性。n阶巴特沃斯低通滤波器的传递函数为：",
            imgExplain1,
        ],
        ["巴特沃斯低通滤波器的示意图", imgExplain2],
    ]
    myWindow.showFig1(picLists)
    placePic1(img_gray, "原灰度图像")
    row, col = img_gray.shape
    ...
    if d0 is None or d0 > maxD0 or d0 < 10:
        showwarning(
            "警告", "截止频率的值应该在[" + str(10) + "~" + str(maxD0) + "]之间"
        )
        return
    nj = askinteger(
        "巴特沃兹低通滤波器参数设置",
        "请设置巴特沃兹低通滤波器的阶数，其值在[1~20]之间的整数",
        initialvalue=1,
    )
...
def Gaussian_low_pass():
    ...
    picLists = [
        [
            "高斯低通滤波器在截止频率较小时，具有较陡峭的频率特征，\n随着截止频率的增大，高斯低通滤波器趋于平缓。",
            imgExplain1,
        ],
        ["高斯低通滤波器的示意图", imgExplain2],
    ]
    myWindow.showFig1(picLists)
    placePic1(img_gray, "原灰度图像")
    row, col = img_gray.shape
    ...
    d0 = askinteger(
        "高斯低通滤波器参数设置",
        "请设置截止频率，其值在[" + str(10) + "~" + str(maxD0) + "]之间",
        initialvalue=(10 + maxD0) // 2,
    )
    if d0 is None or d0 > maxD0 or d0 < 10:
        showwarning(
            "警告", "截止频率的值应该在[" + str(10) + "~" + str(maxD0) + "]之间"
...
def Idle_high_pass():
    ...
    picLists = [
        [
            "理想高通滤波器是将大于截止频率的分量完全\n无损地通过，而小于截止频率的分量完全衰减。",
            imgExplain0,
        ],
        ["理想高通滤波器示意图", imgExplain1],
    ]
    myWindow.showFig1(picLists)
    placePic1(img_gray, "原灰度图像")
    row, col = img_gray.shape
...
def Butterworth_high_pass():
    ...
    picLists = [
        ["巴特沃兹高通滤波器的传递函数为：", imgExplain1],
        ["巴特沃斯高通滤波器的示意图", imgExplain2],
    ]
    myWindow.showFig1(picLists)
    placePic1(img_gray, "原灰度图像")
    row, col = img_gray.shape
...
def Gaussian_high_pass():
    ...
    picLists = [
        ["高斯高通滤波器传递函数为：", imgExplain1],
        ["高斯高通滤波器的示意图", imgExplain2],
    ]
    myWindow.showFig1(picLists)
    placePic1(img_gray, "原灰度图像")
    row, col = img_gray.shape
...
def Move():
    ...
    img_result = geometric.move(
        img_open.copy(), paraW.paraLists[0][1], paraW.paraLists[1][1]
    )
...
def Affine():
    ...
    paraW = paraWindow2(
        root, paraLists, "仿射变换参数设置", "变换前三点坐标     变换后的目标坐标"
    )
...
def Perspective():
    ...
    paraW = paraWindow2(
        root, paraLists, "透视变换参数设置", "变换前四点坐标     变换后的目标坐标"
    )
    paraLists = paraW.paraLists
    pts1 = np.float32(
        [paraLists[0][0:2], paraLists[1][0:2], paraLists[2][0:2], paraLists[3][0:2]]
    )
    pts2 = np.float32(
        [paraLists[0][2:4], paraLists[1][2:4], paraLists[2][2:4], paraLists[3][2:4]]
    )
...
def Interpolation():
    ...
    combinedImg1 = myWindow.combine_images(
        picLists1, explain=True, fontSize=1
    )  # 合并两张图片
    combinedImg2 = myWindow.combine_images(
        picLists2, explain=True, fontSize=1
    )  # 合并两张图片
...
def Add_gaussian_noise():
    ...
    if img_result is None:
        ...
        answer = askyesno(
            "选择待退化图像", "Y-对打开图像进行退化，N-继续对已退化图像进行退化"
        )
        if not answer:
            img_ = img_result.copy()
        else:
    ...
    # paraW.show()
    img_result = spatialRestore.add_gaussian_noise(
        img_, paraW.paraLists[0][1], paraW.paraLists[1][1]
    )
...
def Add_salt_and_pepper_noise():
    ...
    picLists = [
        ["椒盐噪声是一种脉冲噪声，双极脉冲噪声的概率密度函数可以如下表示：", explainPic]
    ]
    myWindow.showFig1(picLists)
...
def Add_mean_noise():
    ...
    # paraW.show()
    img_result = spatialRestore.add_mean_noise(
        img_, paraW.paraLists[0][1], paraW.paraLists[1][1], paraW.paraLists[2][1]
    )
    histr = Histogram.gray_histogram(img_result)
    myWindow.showAndBarFig0(img_result, histr)
...
def Add_rayleigh_noise():
    ...
    img_result = spatialRestore.add_rayleigh_noise(
        img_, paraW.paraLists[0][1], paraW.paraLists[1][1], paraW.paraLists[2][1]
    )
    histr = Histogram.gray_histogram(img_result)
    myWindow.showAndBarFig0(img_result, histr)
...
def Add_erlang_noise():
    ...
    img_result = spatialRestore.add_erlang_noise(
        img_, paraW.paraLists[0][1], paraW.paraLists[1][1], paraW.paraLists[2][1]
    )
    histr = Histogram.gray_histogram(img_result)
    myWindow.showAndBarFig0(img_result, histr)
...
def Motion_blur():
    ...
    picLists = [
        [
            "运动模糊核是由单位矩阵旋转产生的，如下图所示是一个90度方向的运动模糊核",
            explainPic,
        ]
    ]
    myWindow.showFig1(picLists)
    ...
    PSF = Restore.extension_PSF(
        img_, PSF
    )  # 在频域进行运动模糊，需要扩展PSF，使其与图像一样大小
...
def Turbulence_blur():
    ...
    picLists = [
        ["大气湍流模型的传递函数如下图所示，其中k值越大，模糊越严重", explainPic]
    ]
    myWindow.showFig1(picLists)
...
def Contra_harmonic_filter():
    ...
    picLists = [
        [
            "逆谐波均值，Q是滤波器的阶数。Q为负值时，可消除盐噪声。Q为正值时，可消除胡椒噪声。不能同时消除两者",
            imgExplain,
        ]
    ]
    myWindow.showFig1(picLists)
...
def Improved_inverse():
    ...
    picLists = [
        [
            "伪逆滤波的传递函数是：低频部分(小于截止w)频率与逆滤波相同，高频部分采用一个指定的较小的常量k",
            explainPic,
        ]
    ]
    myWindow.showFig1(picLists)
...
def Wiener():
    ...
    picLists = [
        [
            "维纳滤波采用最小均方误差作为最佳过滤准则，使用时需要估计图像的噪信比，无噪声时，噪信比为0",
            explainPic,
        ]
    ]
    myWindow.showFig1(picLists)
...
def Constrained_least_squares():
    ...
    picLists = [
        [
            "约束最小二乘滤波通过抑制高频成份，来获得平滑的复原图像，公式中Q是锐化算子，λ是权重参数，其值越大越平滑",
            explainPic,
        ]
    ]
    myWindow.showFig1(picLists)
...
def Binary():
    ...
    picLists = [
        [
            "图像二值化处理，也就是阈值分割，它的目的就是求一个阈值T，并用T将图像f(x,y)分成对象物和背景两个领域。",
            imgExplain,
        ]
    ]
    myWindow.showFig1(picLists)
...
def Dilate():
    ...
    myWindow.explainText.set(
        "膨胀运算是对二值化物体边界点扩充，将与物体接触的所以背景点合并到该物体中，使边界向外扩张。"
    )
...
def Erode():
    ...
    minrc = min(row, col)
    kernelSize = askinteger(
        "结构算子设置", "请输入结构算子大小", initialvalue=3, minvalue=3, maxvalue=minrc
    )
    paraLists = [["请选择结构算子形状", ["矩形", "十字形", "椭圆形"]]]
    paraW = listWindow(root, "结构算子选项", paraLists)
    structType = paraW.returnValue
...
def Open_operation():
    ...
    myWindow.explainText.set(
        "开运算数学上是先腐蚀后膨胀的结果，物理结果为完全删除了不能包含结构元素的对象区域，平滑了对象的轮廓，断开了狭窄的连接，去掉了细小的突出部分。"
    )
...
def Close_operation():
    ...
    myWindow.explainText.set(
        "闭运算在数学上是先膨胀再腐蚀的结果，物理结果也是会平滑对象的轮廓，但是与开运算不同的是，闭运算一般会将狭窄的缺口连接起来形成细长的弯口，并填充比结构元素小的洞。"
    )
...
def Gradient_morphology():
    ...
    myWindow.explainText.set(
        "形态学梯度是一幅图像膨胀与腐蚀的差别,结果看上去就像前景物体的轮廓。"
    )
...
def Tophat1():
    ...
    minrc = min(row, col)
    kernelSize = askinteger(
        "结构算子设置", "请输入结构算子大小", initialvalue=4, minvalue=3, maxvalue=minrc
    )
    paraLists = [["请选择结构算子形状", ["矩形", "十字形", "椭圆形"]]]
    paraW = listWindow(root, "结构算子选项", paraLists)
    structType = paraW.returnValue
...
def Basic_global_thresholding():
    ...
    myWindow.explainText.set(
        """全局阈值分割就是将整个图像灰度值大于阈值（thresh）的像素调为白色，小于或等于阈值的调整为黑色，也可以反过来。 """
    )
...
def Otsu_threshold():
    ...
    myWindow.explainText.set(
        """以最佳阈值将图像的灰度直方图分割成两部分，使两类之间的方差取最大值，即分离性最大。
    此算法利用了最小二乘法原理。"""
    )
...
def Adaptive_mean_threshold():
    ...
    kernalSize = askinteger(
        "窗口设置", "请输入邻域窗口大小", initialvalue=15, minvalue=3, maxvalue=maxSize
    )
    # img_gray = cv2.GaussianBlur(img_gray, (5, 5), 0)
    img_result = cv2.adaptiveThreshold(
        img_gray, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, kernalSize, 20
    )
...
def Adaptive_gaussian_threshold():
    ...
    maxSize = (min(img_gray.shape[:2])) // 2
    kernalSize = askinteger(
        "窗口设置", "请输入高斯窗口大小", initialvalue=15, minvalue=3, maxvalue=maxSize
    )
    # img_gray = cv2.GaussianBlur(img_gray, (5, 5), 0)
    img_result = cv2.adaptiveThreshold(
        img_gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, kernalSize, 0
    )
...
def Adaptive_mean_threshold1():
    ...
    img_result = cv2.adaptiveThreshold(
        img_, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, kernalSize, 20
    )
    placePic2(img_result, "基于邻域均值的自适应阈值分割图像")
    myWindow.explainText.set(
        "自适应阈值，则是根据像素的邻域块的像素值分布来确定该像素位置上的二值化阈值。"
...
def Adaptive_gaussian_threshold1():
    ...
    # img_ = cv2.GaussianBlur(img_, (5, 5), 0)
    img_result = cv2.adaptiveThreshold(
        img_, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, kernalSize, 18
    )
    placePic2(img_result, "基于高斯滤波的自适应阈值分割图像")
    myWindow.explainText.set(
        "基于高斯滤波的自适应阈值分割，则是根据像素的邻域块的像素值分布来确定该像素位置上的二值化阈值。"
...
def Draw_contours():
    ...
    placePic1(img_open, "原图像")
    myWindow.explainText.set(
        "OpenCV的findContours()函数来查找检测物体的轮廓。OpenCV的drawContours在图像上绘制轮廓。区域面积小于设置最小值的区域将会被忽略"
    )
...
def Get_region_description():
    ...
    img_result, contours = description.get_region_description(
        img_result, contoursMinArea
    )
...
def Moment_invariants1():
    ...
    for tt, img in zip(textLists, imgLists):
        picLists.append([tt, img])
    combinedImg1 = myWindow.combine_images(
        picLists[:3], explain=True, fontSize=0.6
    )  # 合并两张图片
    combinedImg2 = myWindow.combine_images(
        picLists[3:], explain=True, fontSize=0.6
    )  # 合并两张图片
...
def Car_license_train():
    ...
    initWindows()
    myWindow.explainText.set(
        "车牌识别系统通常包括图像预处理、车牌检测定位、字符分割、特征提取和字符识别等部分。"
    ...
    flag = True  # 是否重新训练的标记
    if os.path.exists(r".\carLicense\svm.dat") and os.path.exists(
        r".\carLicense\svmchinese.dat"
    ):
        flag = askyesno(
            "选择是否重新训练模型",
            "模型已经训练过了，训练模型需要花费一段时间，是否重新训练模型？",
        )
        if not flag:
            return
...
def Car_license_recognition():
    ...
    if img_open is None:
        ...
        return
    predictor = predict.CardPredictor(myWindow)  # 创建识别模型
    predictor.stepFlag = askyesno(
        "选择输出结果方式",
        "系统提供了分步显示识别数据和显示最终结果两种显示方式，请选择显示方式：是——分步显示，否——仅显示最终结果",
    )
...
class MyApp:
    ...
    def init_widget(self, master):
        ...
        self.label1 = tkinter.Label(
            master, textvariable=self.originalText, font=("song", 12)
        )
        ...
        self.label2 = tkinter.Label(
            master, textvariable=self.resultText, font=("song", 12)
        )
        self.resultText.set("处理结果图")
        self.label4 = tkinter.Label(master, bg="gray86")  # 用于显示处理结果
        self.regionLabel = tkinter.Label(
            master, text="检测的车牌区域：", font=("song", 12)
        )
        ...
        self.from_pic_ctl = tkinter.Button(
            master,
            text="识别图片",
            width=20,
            command=Car_license_recognition,
            font=("song", 12),
        )
        self.explainText = tkinter.StringVar()
        # 用于显示算法注释说明
        self.label5 = tkinter.Label(
            master,
            textvariable=self.explainText,
            font=("song", 12),
            bg="gray86",
            wraplength=850,
            justify="left",
    ...
    def setVisibleRight1(self):
        ...
        self.roi_ctl.place(
            relx=0.75, rely=0.18, width=picSize, height=50, anchor=tkinter.CENTER
        )
        self.recogResult.place(relx=0.75, rely=0.26, anchor=tkinter.CENTER)
        self.r_ctl.place(
            relx=0.75, rely=0.34, width=picSize, height=50, anchor=tkinter.CENTER
        )
        self.color_ctl.place(
            relx=0.75, rely=0.42, width=picSize, height=30, anchor=tkinter.CENTER
        )
    ...
    def setVisibleFig0(self):
        ...
        myFig0-使用mayplot显示运行结果图片
        """
        self.myCanvas0.get_tk_widget().place(
            relx=0.75, rely=0.40, anchor=tkinter.CENTER
        )
        self.myCanvas0.draw()
    ...
    def setVisibleBottom(self):
        ...
        """
        self.label5.place(
            relx=0.50, rely=0.86, width=900, height=180, anchor=tkinter.CENTER
        )
        self.hideFig1()
        self.hideExplain()
    ...
    def setVisibleExplain(self):
        ...
        """
        self.label5.place(
            relx=0.275, rely=0.86, width=450, height=180, anchor=tkinter.CENTER
        )
        self.label5.config(wraplength=430)
        self.myCanvas2.get_tk_widget().place(
            relx=0.725, rely=0.86, anchor=tkinter.CENTER
        )
        self.myCanvas2.draw()
    ...
    def init_menu(self, master):
        ...
        fmenu_point.add_command(
            label="全局灰度线性变换", command=Global_linear_transmation
        )
        fmenu_point.add_command(
            label="分段线性灰度变换", command=Piecewise_linear_transformation
        )
        fmenu_point.add_command(label="对数变换", command=Logarithmic_transformations)
        fmenu_point.add_command(
            label="幂次（伽马）变换", command=Power_law_transformations
        )
        ...
        submenu_point_1.add_command(
            label="直方图均衡化", command=Equalization_histogram
        )
        ...
        submenu_spatial_3.add_command(
            label="常量（补0）法（BORDER_CONSTANT）", command=Constant_border
        )
        submenu_spatial_3.add_command(
            label="复制法（BORDER_REFLICATE）", command=Replicate_border
        )
        submenu_spatial_3.add_command(
            label="反射法（BORDER_REFLECT）", command=Reflect_border
        )
        submenu_spatial_3.add_command(
            label="反射法1（BORDER_REFLECT_101）", command=Reflect_border_101
        )
        submenu_spatial_3.add_command(
            label="外包装法（BORDER_WRAP）", command=Wrap_border
        )
        ...
        submenu_spectrum_1.add_command(
            label="低频移中的幅值谱", command=Magnitude_spectrum1
        )
        submenu_spectrum_1.add_command(label="相位谱", command=Phase_spectrum0)
        submenu_spectrum_1.add_command(
            label="低频移中的相位谱", command=Phase_spectrum1
        )
        ...
        submenu_spectrum_2.add_command(
            label="巴特沃兹低通滤波器", command=Butterworth_low_pass
        )
        submenu_spectrum_2.add_command(
            label="高斯低通滤波器", command=Gaussian_low_pass
        )
        ...
        submenu_spectrum_3.add_command(
            label="巴特沃斯高通滤波器", command=Butterworth_high_pass
        )
        submenu_spectrum_3.add_command(
            label="高斯高通滤波器", command=Gaussian_high_pass
        )
        ...
        submenu_restore_1.add_command(
            label="添加椒盐噪声", command=Add_salt_and_pepper_noise
        )
        ...
        submenu_restore_3.add_command(
            label="逆谐波滤波", command=Contra_harmonic_filter
        )
        ...
        submenu_restore_6.add_command(
            label="最小二乘滤波", command=Constrained_least_squares
        )
        ...
        fmenu_morphology.add_cascade(
            label="系统默认图像的形态学处理", menu=submenu_morphology1
        )
        ...
        submenu_morphology1.add_command(
            label="形态学梯度", command=Gradient_morphology1
        )
        ...
        fmenu_segment.add_cascade(
            label="系统默认图像的阈值分割", menu=submenu_segment_1
        )
        submenu_segment_1.add_command(
            label="基本全阈值分割", command=Basic_global_thresholding1
        )
        submenu_segment_1.add_command(
            label="最大类间方差OTSU分割", command=Otsu_threshold1
        )
        submenu_segment_1.add_command(
            label="邻域均值自适应阈值分割", command=Adaptive_mean_threshold1
        )
        submenu_segment_1.add_command(
            label="高斯窗口自适应阈值分割", command=Adaptive_gaussian_threshold1
        )
        ...
        fmenu_segment.add_command(
            label="基本全阈值分割", command=Basic_global_thresholding
        )
        fmenu_segment.add_command(label="最大类间方差OTSU分割", command=Otsu_threshold)
        fmenu_segment.add_cascade(
            label="自适应局部可变阈值分割", menu=submenu_segment_2
        )
        submenu_segment_2.add_command(
            label="邻域均值自适应阈值分割", command=Adaptive_mean_threshold
        )
        submenu_segment_2.add_command(
            label="高斯窗口自适应阈值分割", command=Adaptive_gaussian_threshold
        )
        ...
        fmenu_description.add_cascade(
            label="系统默认图像的描述", menu=submenu_description1
        )
        submenu_description1.add_command(label="获取目标轮廓", command=Draw_contours1)
        submenu_description1.add_command(
            label="区域描述子", command=Get_region_description1
        )
        submenu_description1.add_command(
            label="几何变换后的不变矩", command=Moment_invariants1
        )

        fmenu_description.add_command(label="获取目标轮廓", command=Draw_contours)
        fmenu_description.add_command(
            label="区域描述子", command=Get_region_description
        )
        fmenu_description.add_command(
            label="几何变换后的不变矩", command=Moment_invariants
        )
...
class paraWindow(tkinter.Toplevel):
    ...
    def init_widgets(self, master):
        ...
        for i in range(len(self.paraLists)):
            ...
            self.names["paraE" + str(i)] = tkinter.Entry(
                master, textvariable=self.names["paraV" + str(i)], width=20
            )
    ...
    def init_widgets(self):
        ...
        for i in range(len(self.paraLists)):
            ...
            for j in range(len(self.paraLists[i])):
                ...
                self.names["e" + str(i) + str(j)] = tkinter.Entry(
                    frame2, textvariable=self.names["ev" + str(i) + str(j)], width=10
                )
...
class listWindow(tkinter.Toplevel):
    ...
    def init_widgets(self, master):
        ...
        labelMessage = tkinter.Label(
            master, text=self.paraLists[0][0], font=("song", 12)
        )
